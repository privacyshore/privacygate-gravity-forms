...
class GFPrivacyGateAdmin
{
    ...
    public function __construct()
    {
        ...
        if (true === class_exists('GFCommon')) {
            if (version_compare(GFCommon::$version, '1.6.99999', '<')) {
                // pre-v1.7 settings
                $this->settingsURL = admin_url('admin.php?page=gf_settings&addon=PrivacyGate');
            } else {
                // post-v1.7 settings
                $this->settingsURL = admin_url('admin.php?page=gf_settings&subview=PrivacyGate');
            }
        }
    ...
    {
        delete_option(GFPrivacyGateSettings::SETTINGS_OPTIONS);
    }
    ...
    public function admin_notices()
    {
        if (class_exists('RGForms') === false) {
             echo "<div class='error'><p><strong>PrivacyGate plugin for Gravity Forms requires <a href=\"http://www.gravityforms.com/\">Gravity Forms</a> to be installed and activated.</div></p></strong>";
        }
    }
}
