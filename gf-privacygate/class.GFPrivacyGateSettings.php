...
class GFPrivacyGateSettings
{
    const APP_KEY_PARAM = 'app_key';
    const APP_SECRET_PARAM = 'app_secret';
    const SETTINGS_OPTIONS = 'gf_privacygate_options';
    const SETTINGS_PAGE = 'gf_privacygate_settings';
    ...
    function __construct()
    {
        ...
        RGForms::add_settings_page('PrivacyGate', array($this, 'options_page_html'));
    ...
    {
        wp_enqueue_style('gf_privacygate_settings', plugins_url('assets/css/settings.css', __FILE__));
    }
    ...
    function options_page_html()
    {
        if (isset($_GET['settings-updated']) && !get_settings_errors(self::SETTINGS_OPTIONS)) {
            ...
            add_settings_error(self::SETTINGS_OPTIONS, 'gf_privacygate_message', __('Settings Saved', 'heremobility'), 'updated');
        }

        $this->settings_errors(self::SETTINGS_OPTIONS);
        $siteUrl = get_site_url(null, '', 'https');
        $webhookUrl = add_query_arg(array('callback' => GF_PRIVACYGATE_SLUG), $siteUrl);

        ?>
            <div class="settings-wrap">
                <h2>PrivacyGate Settings</h2>
                <div>
                    <h3>How to use</h3>
                    <ul>
                        <li>
                            <p>1. Open the page "Settings" of PrivacyGate Dashboard: <a
                                        href="https://dash.privacygate.io/dashboard/settings">https://dash.privacygate.io/settings</a>
                            </p>
                        </li>
                        <li>
    ...
    function settings_init()
    {
        ...
        $sectionCredentials = 'gf_privacygate_keys';
    ...
    function sanitize_callback($value)
    {
        ...
        if ($error) {
            ...
            add_settings_error(self::SETTINGS_OPTIONS, 'gf_privacygate_message', $message, 'error');
    ...
    function setting_cb($args)
    {
        ...
        <input type="text"
               ...
               name="gf_privacygate_options[<?php echo esc_attr($args['id']); ?>]"
