...
if (method_exists('GFForms', 'include_payment_addon_framework')) {
    ...
    require_once __DIR__ . '/class.GFPrivacyGateAdmin.php';
    require_once __DIR__ . '/class.GFPrivacyGateSettings.php';
    require_once __DIR__ . '/vendor/PrivacyGateSDK/autoload.php';
    require_once __DIR__ . '/vendor/PrivacyGateSDK/const.php';

    /**
     * Class for managing the plugin
     */
    class GFPrivacyGatePlugin extends GFPaymentAddOn
    {
        const RETURN_PAGE_PARAM = 'gf_privacygate_return';
        const FORM_ID_PARAM = 'form_id';
        const LEAD_ID_PARAM = 'lead_id';

        protected $_version = GF_PRIVACYGATE_VERSION;
        protected $_min_gravityforms_version = '1.9';
        protected $_slug = GF_PRIVACYGATE_SLUG;
        protected $_path = 'gf-privacygate/gf-privacygate.php';
        protected $_full_path = __FILE__;
        protected $_title = 'Gravity Forms PrivacyGate Add-On';
        protected $_short_title = 'PrivacyGate Payment';
        ...
        public function init()
        {
            ...
            if (is_admin() == true) {
                new GFPrivacyGateAdmin();
                new GFPrivacyGateSettings();
            }
        }
        ...
        {
            try {
                $apiKey = self::get_privacygate_setting(GFPrivacyGateSettings::APP_KEY_PARAM);
                ...
                $charge = \PrivacyGate\Resources\Charge::create($chargeData);
                ...
                $this->log_debug('Unable to create PrivacyGate charge.' . $exception->getMessage());
                echo __('Unable to complete payment.', 'gf-privacygate');
                die();
            }
        }
        ...
        static function get_privacygate_setting($name)
        {
            $options = get_option(GFPrivacyGateSettings::SETTINGS_OPTIONS);
        ...
        function callback()
        {
            ...
            $apiKey = self::get_privacygate_setting(GFPrivacyGateSettings::APP_KEY_PARAM);
            $sharedSecret = self::get_privacygate_setting(GFPrivacyGateSettings::APP_SECRET_PARAM);

            try {
                $event = \PrivacyGate\Webhook::buildEvent($payload, $signatureHeader, $sharedSecret);
            } catch (\Exception $exception) {
            ...
            \PrivacyGate\ApiClient::init($apiKey);
            $charge = \PrivacyGate\Resources\Charge::retrieve($event->data['id']);
